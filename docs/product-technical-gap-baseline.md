...
Evidence locators remain available through the explicit accessor because authorized evidence drill-through is a product requirement, but default `Debug` output redacts locator content. This protects token-bearing/internal/customer-sensitive locators from routine diagnostic logging without masking the value at the authorized work boundary.
...
| Sensitive evidence diagnostic boundary | this repo | custom `EvidenceReference` debug formatting plus token-bearing locator regression test; full locator remains explicit-accessor only | keep locator redaction default in diagnostics and verify future logging/telemetry never emits explicit locator access by default | debug redaction implemented; production logging surface not yet present | exact-head tests/security scans, then logging/telemetry threat-model tests when service exists |
